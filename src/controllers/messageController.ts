...
export const updateMessage = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (error) throw error;
    broadcastToChat(existing.chat_id, "message updated", data as unknown as Record<string, unknown>);
    res.json(data);
...
export const deleteMessage = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (findError) throw findError;
    if (!existing) { res.status(404).json({ error: "Message not found" }); return; }
    if (existing.sender_id !== userId && !(await isChatAdmin(existing.chat_id, userId))) { res.status(403).json({ error: "Only the sender or a workspace admin can delete it" }); return; }

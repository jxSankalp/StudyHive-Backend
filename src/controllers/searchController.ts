...
import { decodeSearchCursor, encodeSearchCursor, parseSearchLimit } from "../lib/searchCursor";
import { supabase } from "../lib/supabase";

const RESOURCE_TYPES = ["message", "note", "task", "meeting"] as const;
...
export const searchWorkspace = async (req: Request, res: Response): Promise<void> => {
  ...
  const requestedTypes = typeof req.query.types === "string"
    ...
    : [];
  if (requestedTypes.some((item) => !RESOURCE_TYPES.includes(item as ResourceType))) {
    res.status(400).json({ error: "Invalid search filter" });
    return;
  }

  const suppliedCursor = req.query.cursor;
  const cursor = decodeSearchCursor(suppliedCursor);
  ...
  try {
    ...
    res.json({
      ...
      hasMore,
      nextCursor: hasMore && last ? encodeSearchCursor({
